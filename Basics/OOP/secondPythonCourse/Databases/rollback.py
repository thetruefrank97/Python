...
class Account(object):
    ...
    def _save_update(self, amount):
        ...
        withdrawal_time, zone = Account.current_time() # <-- unpack the returned tuple
        picked_zone = pickle.dumps(zone)

        db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)", (new_balance, self.name))
        db.execute("INSERT INTO history VALUES(?, ?, ?, ?)", (withdrawal_time, self.name, -amount, picked_zone))
        db.commit()
    ...
    def deposit(self, amount: int) -> float:
        if amount > 0.0:
            # new_balance = self._balance + amount
            # deposit_time = Account.current_time()
            # db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)", (new_balance, self.name))
    ...
    def withdraw(self, amount: int) -> float:
        if 0 < amount <= self._balance:
            # new_balance = self._balance - amount
            # withdrawal_time =  Account.current_time()
            # db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)",(new_balance, self.name))
    ...
    def show_balance(self):
        print("Blance on account {} is {} ".format(self.name, self._balance / 100))
